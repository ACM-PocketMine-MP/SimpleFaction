...
class FactionsAPI {
     ...
     * @return bool
     */
    public static function isInClaim(World $level, int $chunkX, int $chunkZ): bool {
     ...
     * @return string
     */
    public static function getFactionClaim(World $level, int $chunkX, int $chunkZ): string {
        $world = $level->getFolderName();
        foreach (self::$claim as $faction => $claims) {
    ...
    public static function getMap(Player $player): array {
        ...
        for ($dz = 0; $dz < $height; $dz++) {
            ...
            for ($dx = 0; $dx < $width; $dx++) {
                $chunkX = (int)$center->getX() - ($width / 2) + $dx;
                $chunkZ = (int)$center->getZ() - ($height / 2) + $dz;
                if ($chunkX === (int)$center->getX() && $chunkZ === (int)$center->getZ()) {
